var App = new Ext.Application({
	...
	launch: function() {
		...
		store: new Ext.regStore('FlashcardsStore', {
			...
			getGroupString: function (record) {
				if (record && record.data.word) {
					return record.get('word')[0].toUpperCase();
				} else {
					return '';
				}
			}
		...
		flashcardGameTopToolbar = new Ext.Toolbar({
			id: 'flashcardGameTopToolbar',
			title: 'Learn Card',
			items: [
				{
					text: 'Home',
		...
		flashcardGameBottomToolbar = new Ext.Toolbar({
			id: 'flashcardGameBottomToolbar',
			dock: 'bottom',
			items: [
					// Next/Previous Flashcard
					// Block Previous if it is first, block Next if it is last
			]
		});
		...
		var viewport = new Ext.Panel({
			...
			layout: 'card',
			cardAnimation: 'slide',
			items: [flashcardListContainer, flashcardEditor, flashcardGame]
		});
	}
});